...
bool UDBDUtilities::IsClassChildOfOtherClasses(const UClass* classToCheck, const TArray<UClass*>& otherClasses)
{
	return false;
}
...
float UDBDUtilities::GetSquaredDistanceToKiller(const AActor* actor)
{
	return 0.0f;
}
...
bool UDBDUtilities::FindFloorSingle(const UObject* WorldContextObject, const FVector& location, float raycastLength, FHitResult& outHit)
{
	return false;
}
